...
''' this is new enhanced code which gives the better quality scarpping
gives population boolean value but working fine '''
...
#                 if not (EXPOSURE_PATTERN.search(text) and INTERVENTION_PATTERN.search(text)):
#                     continue
#                 found_exposures = [m.group(0) for m in EXPOSURE_PATTERN.finditer(text)]
#                 found_interventions = [m.group(0) for m in INTERVENTION_PATTERN.finditer(text)]
#                 found_population = [m.group(0) for m in POPULATION_PATTERN.finditer(text)]
#                 pop_flag = "Yes" if found_population else "No"
...
#                      "Country":art.findtext(".//MedlineJournalInfo/Country",""),
#                      "Type":"; ".join([pt.text for pt in art.findall(".//PublicationType") if pt.text]),
#                      "Exposure_Matched":", ".join(set(found_exposures)),
#                      "Intervention_Matched":", ".join(set(found_interventions)),
#                      "Population_Matched":pop_flag,
#                      "Extra":f"PICO found -> Exposure: {', '.join(set(found_exposures))}; Intervention: {', '.join(set(found_interventions))}; Population: {pop_flag}"}
#                 self.save_record(rec,[normalize_title(title), doi, pmid])
...
#     def dedup_identifier(self, rec):
#         """Prioritize deduplication by Title > DOI > PMID"""
#         title_id = normalize_title(rec.get("Title",""))
#         doi = rec.get("DOI","")
#         pmid = rec.get("Key","")
#         return [title_id, doi, pmid]

#     def save_record(self, rec):
#         identifiers = self.dedup_identifier(rec)
#         title_id = identifiers[0]
    
#     # Check if any identifier exists already
#         cur=self.store.conn.cursor()
#         cur.execute("SELECT data FROM recs WHERE identifier=? OR identifier=? OR identifier=?", (identifiers[0], identifiers[1], identifiers[2]))
#         row = cur.fetchone()
...
#                     f"SKIPPED DUPLICATE: '{rec.get('Title','')}' ({rec.get('DOI','')}) "
#                     f"matched existing '{existing.get('Title','')}' ({existing.get('DOI','')}) "
#                     f"from {rec.get('Url','')}"
#                 )   
#             self.skipped_count += 1
#             return False
    
#         ident = title_id or identifiers[1] or identifiers[2]
#         self.store.insert(ident, rec)
#         self.added_count += 1
#         return True
...
#                 found_interventions = [m.group(0) for m in INTERVENTION_PATTERN.finditer(text)]
#                 found_population = [m.group(0) for m in POPULATION_PATTERN.finditer(text)]
#                 pop_flag = ", ".join(set(found_population)) if found_population else ""
...
#                      "Intervention_Matched":", ".join(set(found_interventions)),
#                      "Population_Matched":pop_flag,
#                      "Extra":f"PICO -> Exposure: {', '.join(set(found_exposures))}; Intervention: {', '.join(set(found_interventions))}; Population: {pop_flag}"}
...
#     store=Storage();fetcher=Fetcher();scraper=GeneralScraper(fetcher,store,merge_report)
#     for src in sources:
#         if src.strip()=="pubmed": scraper.scrape_pubmed(query,limit,from_year,to_year)
#         elif src.strip()=="crossref": scraper.scrape_crossref(query,limit,from_year,to_year)
#     recs=store.all();export(recs);scraper.finalize();store.close()
...
# -*- coding: utf-8 -*-
"""
CardioProtect MultiSource Scraper – Strict PICO, Human-Only
...
def detect_study_design(text):
    """
    Detects if text describes an RCT or observational study.
    Explicitly EXCLUDES:
      - Systematic / meta-analyses
      - Generic narrative / state-of-the-art reviews
      - Secondary / pooled / post-hoc analyses
    Returns (ok, design_label)
    """
    text_low = text.lower()

    # 1. HARD EXCLUDE: clearly labeled systematic / meta-analytic reviews
    hard_review_terms = [
        "systematic review", "meta-analysis", "meta analysis",
        "network meta-analysis", "network meta analysis",
        "scoping review", "narrative review", "literature review",
        "overview of reviews", "umbrella review", "cochrane review",
        "current perspectives", "current perspective",
        "state of the art", "update on", "current concepts"
    ]
    if any(ph in text_low for ph in hard_review_terms):
        return False, "Excluded (Review/Meta-analysis)"

    # 2. TRIAL / COHORT SIGNALS
    rct_terms = [
        "randomized", "randomised", "placebo", "double blind", "single blind",
        "clinical trial", "randomly assigned", "intervention arm", "controlled trial",
        "parallel-group", "multicenter trial", "multicentre trial"
    ]
    obs_terms = [
        "cohort", "prospective", "retrospective", "registry", "case-control",
        "case control", "observational", "follow-up study", "follow up study",
        "real-world", "real world", "population-based", "population based"
    ]

    is_rct = any(t in text_low for t in rct_terms)
    is_obs = any(t in text_low for t in obs_terms)

    # 3. SOFT EXCLUDE: clear “review-ish” language when there is NO trial signal
    #    (To catch things like "current perspectives", "state of the art", etc.)
    soft_review_terms = [
        "review",              # only used when NOT rct/obs
    ...
    if not is_rct and not is_obs and any(ph in text_low for ph in soft_review_terms):
        return False, "Excluded (Narrative/Overview Review)"

    # 4. SECONDARY / POOLED / POST-HOC ANALYSES (not primary RCT/obs for your template)
    secondary_terms = [
        "secondary analysis", "post hoc analysis", "post-hoc analysis",
        "pooled analysis", "pooled data analysis",
        "meta-regression", "meta regression",
        "individual patient data meta-analysis",
        "ipd meta-analysis", "systematic search"
    ]
    if any(ph in text_low for ph in secondary_terms):
        return False, "Excluded (Secondary/Meta/Pooling)"
    ...
    # Everything else: not usable as primary study
    return False, "Other"
...
class GeneralScraper:
    ...
    # ---------- NEW: use PubMed PublicationType to drop reviews ----------
                pub_types = [
                    (pt.text or "").lower()
                    for pt in art.findall(".//PublicationTypeList/PublicationType")
                    if pt.text
                ]
                if any("review" in pt or "meta-analysis" in pt or "meta analysis" in pt
                        for pt in pub_types):
                    log.debug(f"Skipping PubMed review/meta: {title[:100]} | types={pub_types}")
                    continue
    # ---------------------------------------------------------------------
    ...
    # ✅ Require explicit RCT or observational study
                design_ok, design_label = detect_study_design(text)
                if not design_ok:
                    log.debug(f"Skipping non-RCT/Observational: {title[:100]}")
                    continue
    ...
    # ---------- NEW: use CrossRef 'type' to drop obvious reviews ----------
                cross_type = (it.get("type") or "").lower()
                if any(x in cross_type for x in ["review", "meta-analysis", "meta_analysis"]):
                    log.debug(f"Skipping CrossRef review/meta: {title[:100]} | type={cross_type}")
                    continue
    # ----------------------------------------------------------------------
...
# ---------------- Run ----------------
def run(query,sources,limit,from_year,to_year,merge_report=False,db_path="scraper_results.db"):
    store=Storage(path=db_path);fetcher=Fetcher();scraper=GeneralScraper(fetcher,store,merge_report)
    for s in (sources or []):
        s=s.strip().lower()
...
if __name__=="__main__":
    ...
    p.add_argument("--to_year",type=int)
    p.add_argument("--merge_report",action="store_true")
    p.add_argument("--db_path", default="scraper_results.db") 
    a=p.parse_args()
    run(a.query,a.sources.split(","),a.limit,a.from_year,a.to_year,a.merge_report,a.db_path)
